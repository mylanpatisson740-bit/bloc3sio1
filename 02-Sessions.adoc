...
== Partie 1 : Activité OWASP (Authentification & Sessions)

=== I. Présentation générale

==== 1. Le risque Broken Authentication
...
=== II. Objectifs et architecture de l’activité
...
* **Machine attaquante :** 192.168.0.11 (Navigateur + Proxy Burp Suite)
* **Serveur web :** 192.168.0.10 (Mutillidae / OWASP)
...
==== 2. À vous de jouer (Voir Dossier 1 pour le pas-à-pas)

**A. Mode non sécurisé (Level 0)**
...
==== 2. À vous de jouer (Voir Dossier 2 pour le pas-à-pas)
...
==== 2. À vous de jouer (Voir Dossier 3 pour le pas-à-pas)
...
=== VI. Conclusion : Tableau de synthèse
...
* Onglet **Options** (Grep - Extract) : Ajouter un filtre.

.Ajout d'un filtre Grep - Extract
image::02-Step3-5.png[Options : Bouton Add Grep Extract]

* Définir l'expression à extraire : "Results for".

.Définition de l'expression
image::02-Step3-6.png[Filtre : Start after expression 'Results for']
...
== Partie 2 : Challenge cURL (Session)

=== I. Le Challenge

Voici la transcription du challenge session :

. **Installation :** Installez les 2 fichiers `session.php` et `supprime.php` (situés dans le répertoire challenge fourni) dans un répertoire visible par Laragon.
...
=== IV. Annexes : Installation de Curl

**Outil en ligne de commande - Curl**

. Vous pouvez dézipper le programme fourni `curl-8.11.1_3-win64-mingw.zip` dans le répertoire `c:\Programmes`.
. Ensuite vous pouvez ajouter le répertoire `bin` à la variable d’environnement `PATH`.
+
Exemple de chemin à ajouter :
`C:\Program Files\curl-8.4.0_6-win64-mingw\bin`
